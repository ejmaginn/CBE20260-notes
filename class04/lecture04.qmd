...
This is one of the most practical ideas in thermodynamics:
...
A common strategy is to split a change into two steps—e.g., first change $P$ at constant $T$, then change $T$ at constant $P$ (or vice versa). For example,
$$
\Delta X
= X(T_2,P_2) - X(T_1,P_1)
$$
...
These depend on the process details (path), not just end states. (We will make this precise as we proceed.)

---

##  Gibbs phase rule

State specification becomes especially clear when phases are present.
...
- **Two phases** ($\Pi=2$), e.g., liquid + vapor:
  ...
  F = 1 - 2 + 2 = 1
  $$
  Only one independent intensive variable can be chosen.  
  Example: specifying $T$ fixes the saturation pressure $P_{\text{sat}}(T)$.

- **Three phases** ($\Pi=3$), solid + liquid + vapor:
  $$
  F = 1 - 3 + 2 = 0
  $$
  No freedom: the system must be at the **triple point**.
...
<!-- ![Typical P–T phase diagram showing triple point, critical point, and phase regions.](images/lecture04/pt_phase_diagram.png){#fig-pt-diagram width=85%}
-->
### 5.1 Triple point
...
<!--
![Boiling point decreases with elevation because atmospheric pressure decreases.](images/lecture04/boiling_point_elevation.png){#fig-bp-elevation width=75%}
-->
...
A typical set of isotherms looks like:

<!--
![Typical P–V behavior for a pure substance showing saturated liquid/vapor lines and the critical point.](images/lecture04/pv_isotherms.png){#fig-pv-isotherms width=85%}
-->
...
### 6.2 Quality (two-phase mixtures)
...
The P–T and P–V diagrams are **projections** of a more complete picture: the **3D P–V–T surface**.

<!--
![Schematic 3D P–V–T surface showing solid, liquid, vapor regions, critical point, and a triple line.](images/lecture04/pvt_surface.png){#fig-pvt-surface width=80%}
-->
...
1. For a pure substance, evaluate $F$ for:
   - single-phase liquid
   - liquid + vapor
   - solid + liquid + vapor

2. On a P–T diagram, explain why the boiling point at a given pressure is a *single* temperature.

3. On a P–V diagram, explain what changes (and what does not change) as a system moves through the two-phase region at fixed temperature.
