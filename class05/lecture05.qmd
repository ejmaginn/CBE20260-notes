...
Heat capacity describes how molecules "store" energy. Recall from our discussion of the Maxwell-Boltzmann equation that the average translational kinetic energy of a simple molecule is
$$
\left\langle \tfrac{1}{2} m v^2 \right\rangle
= \tfrac{3}{2} k_B T
$$

That means that each "direction" of motion (x, y, z) can store $1/2 R$ kinetic energy. We therefore expect that if a molecule only can store translational kinetic energy (a good approximation for spherical molecules like noble gases), 
...
If instead of a simple spherical molecule (like argon), the molecule is a diatomic and the bond behaves like a harmonic spring, then it turns out that the vibratinoal mode of the bond can store and additional $R$ of energy. To a good approximation, foro simple diatomics like $N_2$ and $O_2$, we can estimate the ideal gas heat capacity as
...
Larger molecules will have laregr heat capacities, since they have more modes to store energy. 
